...
class SnapsterBot:
    ...
    async def extract_user_id(self):
        try:
            ...
            log(f"An error occured when getting user_id: {e}")
            return None
                ...
                async with session.get(url, headers=self.headers, proxy=proxy) as response:
                    if response.status == 200:
                        ...
                        if data.get('result'):
                            ...
                            league_bonus = user_data.get('currentLeague').get('bonusPoints')
                            log(hju + f"Username: {pth}{username}")
                            log(hju + f"Points: {pth}{points_count} {hju}| League : {pth}{league} {hju}| Bonus : {pth}{league_bonus}")
                        else:
                            log(mrh + f"Failed to fetch user data.")
                    else:
                        log(mrh + f"Error: {response.status}")
            except Exception as e:
                log(mrh + f"An occured error: {e}")
                    ...
                    if response.status == 200:
                        ...
                        if data.get('result'):
                            ...
                            if 'Not possible to start' in error_message:
                                log(hju + f"Streak: {pth}{data.get('result')} {hju}| {kng}You have already checkin today")
                            else:
                                log(mrh + f"Failed to claim daily bonus: {error_message}")
                    else:
                        log(mrh + f"Error claiming daily bonus: {htm}{response.status}")
            except Exception as e:
                log(mrh + f"Error while claiming daily bonus: {htm}{e}")
    ...
    async def claim_league_bonus(self, session, proxy):
        ...
        async with aiohttp.ClientSession() as session:
            try:
                ...
                log(mrh + f"Error while getting league info: {e}")
    ...
    async def _claim_league_bonus(self, session, proxy, league_id):
        ...
        try:
            ...
            return False, f"Error claiming league bonus: {e}"
                ...
                async with session.post(url, headers=self.headers, json=payload, proxy=proxy) as response:
                    if response.status == 200:
                        ...
                        if data.get('result'):
                            ...
                            log(hju + f"Success claimed Mining | {bru}Points: {pth}{points_claimed} {hju}| Speed: {pth}{mining_speed}x")
                        else:
                            log(mrh + f"Failed! {data.get('message', 'Unknown error')}")
                    else:
                        log(mrh + f"Error: {response.status}")
            except Exception as e:
                log(mrh + f"An error occurred while claiming the mining bonus: {e}")
                        ...
                        if data.get('result'):
                            ...
                            for task in tasks:
                                ...
                                if task_status == 'EARN':
                                    ...
                                    result, message = False, htm + f"COMPLETE"
                                
                                log(hju + f"Tasks {pth}{task_name}")
                                log(bru + f"Status: {hju}{task_status} - {kng}{'Started' if task_status == 'EARN' else 'Already claimed'} {hju}| {pth}{message}")
                        else:
                            log(mrh + "Failed to fetch tasks list")
                    else:
                        log(mrh + f"{response.status}")
            except Exception as e:
                log(mrh + f"An error occurred when getting tasks list: {e}")
    ...
    async def start_quest(self, session, proxy, quest_id):
        ...
        try:
                ...
                if response.status == 200:
                    ...
                    if data.get('result'):
                        ...
                        if 'Referral Count threshold' in error_message:
                            return False, mrh + f"Referral not achieved."
            ...
            return False, mrh + f"An error occurred when starting the quest: {htm}{e}"
    ...
    async def claim_quest_bonus(self, session, proxy, quest_id):
        ...
        try:
            ...
            return False, mrh + f"An error occurred while claiming the quest bonus: {e}"
    ...
    async def claim_referral_points(self, session, proxy):
        ...
        async with aiohttp.ClientSession() as session:
            try:
                ...
                log(f"Exception occurred while claiming referral points: {e}")
            ...
            try:
                ...
                for query_id in query_ids:
                    ...
                    async with aiohttp.ClientSession(connector=connector) as session:
                        log(hju + f"Account #{account_number + 1}")
                        log(htm + "~" * 38)
